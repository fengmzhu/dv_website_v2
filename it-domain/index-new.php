...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_project'])) {
        try {
            $stmt = $db->prepare("INSERT INTO it_domain_projects (
            ...
            $message = showAlert('Project added successfully! Task index auto-generated.', 'success');
    ...
    if (isset($_POST['update_project'])) {
        try {
            $stmt = $db->prepare("UPDATE it_domain_projects SET 
                project_name = ?, spip_ip = ?, ip = ?, ip_postfix = ?, ip_subtype = ?, alternative_name = ?,
                spip_url = ?, wiki_url = ?, spec_version = ?, spec_path = ?,
...
$projects = $db->query("SELECT * FROM it_domain_projects ORDER BY task_index")->fetchAll();

// Get project for editing
$edit_project = null;
if ($action === 'edit' && $id) {
    $stmt = $db->prepare("SELECT * FROM it_domain_projects WHERE id = ?");
    $stmt->execute([$id]);
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td><code><?php echo htmlspecialchars($project['task_index']); ?></code></td>
                                            ...
                                            <td>
                                                <span class="badge bg-<?php echo $project['ip_subtype'] === 'default' ? 'secondary' : 'primary'; ?>">
                                                    <?php echo htmlspecialchars($project['ip_subtype']); ?>
                                                </span>
                                            </td>
                                            ...
                                            <td>
                                                <div class="btn-group" role="group">
                                                    <a href="?action=edit&id=<?php echo $project['id']; ?>" class="btn btn-sm btn-outline-primary">Edit</a>
                                                    <a href="?action=view&id=<?php echo $project['id']; ?>" class="btn btn-sm btn-outline-success">View</a>
                                                </div>
                                            </td>
                                        </tr>
                                        <?php endforeach; ?>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control" id="ip_subtype" name="ip_subtype">
                                            <option value="default" <?php echo ($edit_project && $edit_project['ip_subtype'] === 'default') ? 'selected' : ''; ?>>default</option>
